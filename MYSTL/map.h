...
// 头文件包括了 map 和 multi-map
// map : 具有键值和实值，会根据键值大小自动排序，键值不允许重复
// multimap: 具有键值和实值，会根据键值大小自动排序，键值允许重复
...
public:
    ...
    {   
        ...
        // 如果it == end  或者 it->first < key 说明找不到
    ...
    {   // it->first >= key;
        iterator it = lower_bound(key);
        // 如果it ==end() 或者 key 严格小于 it->first,则需要进一步查找
        if(it == end() || key_compare()(it->first,key))
        // 按照 位置提示it 取寻找对应的value
        // 第一次 定义it 已经是it->first >= key, 
        // 第二次 判断 it->first 严格大于 key 
        // 所以只需要判断key 是否在it 和 it 前一个位置之间即可。
